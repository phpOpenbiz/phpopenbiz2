...
function fillDB()
{
   ...
   $queryFile = "trac.sql";
	...
	if (!$fp) {
		echo 'ERROR: Unable to open $queryFile!';
		return;
	}
...
function replaceDbConfig()
{
   ...
   $xml->DataSource->Database[1]['Driver'] = $_REQUEST['dbtype'];
   $xml->DataSource->Database[1]['Server'] = $_REQUEST['dbHostName'];
   $xml->DataSource->Database[1]['User'] = $_REQUEST['dbUserName'];
   $xml->DataSource->Database[1]['Password'] = $_REQUEST['dbPassword'];
   $xml->DataSource->Database[1]['DBName'] = $_REQUEST['dbName'];
...
function create_db()
{
    ...
    new Ajax.Request('index_trac.php?action=create_db', {
      onLoading: function() {
...
function fill_db()
{
    ...
    new Ajax.Request('index_trac.php?action=fill_db', {
      onLoading: function() {
...
{
   new Ajax.Request('index_trac.php?action=replace_db_cfg', {
      onLoading: function() {
...
function finish()
{
   window.location = "bin/controller.php?view=trac.VW_Ticket";
}
//-->
</SCRIPT>
</head>
...
<h2>Welcome to the Trac Setup Wizard</h2>
<p>
This installer creates the Trac database tables and sets the configuration files that you need to start.
</p>
<p style="color:blue">
<b>I have done the installation, <a href="bin/controller.php?view=trac.VW_Ticket">bring me to home</a>.</b>
</p>
...
<form id="setupform" name="setupform" method="post" action="index_trac.php">
<table  border="1">
<tr>
...
<tr>
	<td>Database Name</td>
	<td><input type="text" name="dbName" value="<?php echo  isset($_REQUEST['dbName']) ? $_REQUEST['dbName'] : 'trac'?>" tabindex="3"></td>
</tr>
<tr>
...
$status[5]['item'] = 'Application files path';
